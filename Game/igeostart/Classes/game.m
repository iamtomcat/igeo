...
-(void) ccTouchEnded:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	if (tapCount == 2) {
		...
		unsigned int test = [namepop tileGIDAt:ccp((int)(addtest.x / (int)(20*zoomFactor)),((mapLayerSize.height-1) - (int)addtest.y / (int)(20*zoomFactor)))];
...
{	
	...
	heightPixels = (((mapLayerSize.height*(20*zoomFactor))-320)+8);
	widthPixels =	(((mapLayerSize.width*(20*zoomFactor))-480)+8);
...
-(id) init
{
	...
	if( (self=[super init] )) {
		...
		TMXLayer *mapLayer = [map layerNamed:@"tile_id"];
		...
		[self schedule:@selector(questionGenerator:) interval:1.2f];
	}
	return self;
}
...
-(void)questionGenerator:(ccTime)dt
{
	...
	if (questionAns==FALSE)
	{
		...
		//output = [NSString stringWithFormat:@"Where is: %?",save];	
