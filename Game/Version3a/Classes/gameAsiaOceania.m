...
@implementation gameAsiaOceania
...
+(id) scene
{
	...
	gameAsiaOceania *layer = [gameAsiaOceania node];
...
{	
	...
	if ((currentPos.x>-(6120*zoomFactor))&&(currentPos.y>8)) 
	{
		[node runAction:[MoveTo actionWithDuration: 0.2f position:ccp(-(6130*zoomFactor), 0)]];
	}
	//check for top left corner
	else if ((currentPos.x>-(6120*zoomFactor))&&(currentPos.y<-heightPixels)) 
	{
		[node runAction:[MoveTo actionWithDuration: 0.2f position:ccp(-(6130*zoomFactor), -(heightPixels-8))]];
	}
	...
	else if (currentPos.x>-(6120*zoomFactor)) 
	{
		[node runAction:[MoveTo actionWithDuration: 0.2f position:ccp(-(6130*zoomFactor), currentPos.y)]];
	}
