...
+(id) scene
{
	...
	gameWholeWorld *layer = [gameWholeWorld node];
...
-(void) setStage
{
	...
	{
		query = [NSString stringWithFormat: @"SELECT * FROM worldMap WHERE continent = %d AND", continent];
	}
	if (stage < 4)
	{
		NSString *stageString = [NSString stringWithFormat: @" WHERE difficulty = " "%d ORDER BY Random() LIMIT 10", stage];
