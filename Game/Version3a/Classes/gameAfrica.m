...
+(id) scene
{
	...
	gameAfrica *layer = [gameAfrica node];
...
{	
	...
	heightPixels = (3072*zoomFactor);
	//widthPixels =	(((mapLayerSize.width*(32*zoomFactor))-480)+8);
	widthPixels = (6356*zoomFactor);
	
	//Check for bottom left corner
	if ((currentPos.x>-(4284*zoomFactor))&&(currentPos.y>8)) 
	{
		[node runAction:[MoveTo actionWithDuration: 0.2f position:ccp(-(4300*zoomFactor), 0)]];
	}
	//check for top left corner
	else if ((currentPos.x>-(4284*zoomFactor))&&(currentPos.y<-heightPixels)) 
	{
		[node runAction:[MoveTo actionWithDuration: 0.2f position:ccp(-(4300*zoomFactor), -(heightPixels-8))]];
	}
	...
	else if (currentPos.x>-(4284*zoomFactor)) 
	{
		[node runAction:[MoveTo actionWithDuration: 0.2f position:ccp(-(4300*zoomFactor), currentPos.y)]];
	}
	//bottom check to make sure user can't go down too far
